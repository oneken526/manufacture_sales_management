...
`resources/views/ui-catalog.blade.php` が存在する場合は、そのファイル **1件のみ** を読み込み、
既存コンポーネントの名前・props・バリアントを把握する。

カタログ内の以下のパターンを読み取る:
- コンポーネント名: `font-mono` クラスを持つカードヘッダーの `x-xxx.yyy` 形式のテキスト
- props / バリアント一覧: 同ヘッダー内の `props:` 以降の記述

カタログが存在しない場合（初回実行時）のみ、`resources/views/components/` 以下の全 `.blade.php` を読み込む。
その場合もディレクトリが空・存在しない場合はスキップする。

> **なぜカタログを使うか**: コンポーネント数が増えても読み込みファイル数が1件で固定され、
> ステップ1の処理時間が増加しない。カタログは Step 6 で常に最新状態に保たれる。
...
- 各セクションには以下を含める:
  ...
  3. 各カードのヘッダー直下に説明文（1〜2文）
  4. 各カードの末尾に使用例コード（`<pre><code>` タグ）
...
各コンポーネントカードのヘッダー行（`font-mono` の div）と preview エリアの**間**に説明行を挿入する。

```html
<div class="px-5 py-2.5 text-xs text-slate-500 border-b border-slate-100">説明文</div>
```

説明文に含める内容（1〜2文）:

1. **何に使うか** — どの画面・状況で使うコンポーネントか
2. **使い分け** — variant / type などの切り替え基準（存在する場合）
3. **注意点** — 必須 props、セットで使うコンポーネント、Blade エスケープ等（必要な場合のみ）

インラインコード（prop 名・タグ名・クラス名）は `<code class="bg-slate-100 px-1 rounded">` で装飾する。

### 使用シーンタグの追加ルール

各コンポーネントカードのヘッダー div を以下の構造に変更し、コンポーネント名の右に使用シーンタグを付与する。

```html
<div class="px-5 py-2 bg-slate-50 border-b border-slate-100 flex items-center gap-2 flex-wrap">
    <span class="text-xs text-slate-400 font-mono">x-buttons.xxx</span>
    <span class="text-xs bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded font-medium">フォームページ</span>
</div>
```

タグは以下の4種類から1つ以上を選んで付与する。複数のシーンで使う場合は複数付与する。

| タグ | 色 | 付与する目安 |
|------|----|----|
| フォームページ | `bg-indigo-50 text-indigo-600` | 登録・編集フォームで使うボタン・入力 |
| 検索バー | `bg-emerald-50 text-emerald-600` | 一覧ページの検索フォームで使う要素 |
| 一覧ページ | `bg-amber-50 text-amber-600` | テーブル・一覧画面で使うボタン・バッジ |
| ダッシュボード | `bg-violet-50 text-violet-600` | ダッシュボード専用のカード・リンク |

また、ページヘッダーの凡例（`最終更新:` の行）にも同じタグを並べて表示すること。
