...
## 問題C-1 — app.css にあるが未使用（全ファイルスキャン時のみ）
...
## 問題C-2 — `<style>` タグに定義されているが同ファイル内の HTML で未使用

| クラス名 | ファイル | 備考 |
|---|---|---|
| `.prd-note` | `products/index.blade.php` | `<style>` に定義済み、HTML 内の `class=` 属性に一切登場しない |
| `.user-status-label` | `users/index.blade.php` | `<style>` に定義済み、HTML 内の `class=` 属性に一切登場しない |

**検出のポイント:**
- 疑似セレクタ（`:hover` 等）は除去してベースクラス名で判定する
- `@include` を含むファイルの場合は子テンプレート参照の可能性があるため `⚠️ 要確認` フラグを立てる（今回の2件は `@include` なしのため確定削除候補）
