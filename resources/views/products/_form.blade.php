...
<div class="grid grid-cols-1 gap-4">
    ...
    <div>
        ...
        <select name="category_id"
                class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 {{ $errors->has('category_id') ? 'border-red-400' : 'border-gray-300' }}">
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="number" name="unit_price" min="0" step="1"
                   ...
                   class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 {{ $errors->has('unit_price') ? 'border-red-400' : 'border-gray-300' }}">
            @error('unit_price')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" name="unit_name"
                   ...
                   class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 {{ $errors->has('unit_name') ? 'border-red-400' : 'border-gray-300' }}">
            @error('unit_name')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
    ...
    <div>
        ...
        <textarea name="notes" rows="3"
                  class="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 {{ $errors->has('notes') ? 'border-red-400' : 'border-gray-300' }}">{{ old('notes', $product?->notes) }}</textarea>
        @error('notes')
            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
        @enderror
    </div>
